  ...
  return <dialog ref={dialog} className="settings-dialog" aria-labelledby="settings-title" onCancel={e => { e.preventDefault(); close(); }} onClick={e => { if (e.target === dialog.current) close(); }}>
    ...
    <form ref={form} onSubmit={e => { e.preventDefault(); save(); }}><fieldset disabled={busy}>
      <div className="settings-body">
        <section className="pairing-guide"><h3>Pair with your PVT desktop</h3><ol><li><strong>Save this remote’s pairing file.</strong><p>Open it in PVT’s Settings → Remotes.</p></li><li><strong>Open PVT’s pairing file here.</strong><p>Save changes below to connect automatically.</p></li></ol><div className="button-row"><button type="button" onClick={onExport}>Save this Remote’s pairing file</button><button type="button" onClick={() => file.current.click()}>Open PVT pairing file</button><input ref={file} type="file" accept=".pvthost,application/json" hidden onChange={e => { const input = e.target.files[0]; e.target.value = ''; importHost(input); }}/></div></section>
