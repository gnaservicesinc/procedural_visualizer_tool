...
export function Settings({data, display, onSave, onClose, onExport, store}) {
  ...
  const importHost = async input => {
    ...
    setBusy(true); setError('');
    try {
      if (input.size > 16384) throw Error('PVT host files must be smaller than 16 KiB.');
  ...
  return <dialog ref={dialog} className="settings-dialog" aria-labelledby="settings-title" onCancel={e => { e.preventDefault(); close(); }} onClick={e => { if (e.target === dialog.current) close(); }}>
    ...
    <form ref={form} onSubmit={e => { e.preventDefault(); save(); }}><fieldset disabled={busy}>
      <div className="settings-body">
        <section className="pairing-guide"><h3>Pair with your PVT desktop</h3><ol><li><strong>Export this remote’s file (.pvtremote).</strong><p>Open it in PVT’s Settings → Remotes.</p></li><li><strong>Import PVT’s host file (.pvthost) here.</strong><p>Save changes below to connect automatically.</p></li></ol><div className="button-row"><button type="button" onClick={onExport}>Export Remote file (.pvtremote)</button><button type="button" onClick={() => file.current.click()}>Import PVT host file (.pvthost)</button><input ref={file} type="file" accept=".pvthost,application/json" hidden onChange={e => { const input = e.target.files[0]; e.target.value = ''; importHost(input); }}/></div></section>
        <section className="companion-extension"><h3>{display ? 'PVT Remote Display' : 'PVT Remote Control'}</h3><p>{display ? 'Show PVT’s live video and audio in this browser.' : 'Adjust PVT’s project and layer controls from this browser.'}</p><a href={chromeStores[display ? 'control' : 'display']} target="_blank" rel="noopener noreferrer">{display ? 'Also available: Remote Control' : 'Also available: Remote Display'} on the Chrome Web Store ↗</a></section>
        <section className="paired-hosts"><h3>Paired hosts <span className="muted">{hosts.length}</span></h3><p>Give each desktop a name you recognize.</p>
          {!hosts.length && <p className="notice">No paired hosts yet. Import a PVT host file (.pvthost) to get started.</p>}
          <ul className="hosts">{hosts.map(host => <li key={host.id}><div className="host-profile"><label>Host name<input required maxLength={120} value={host.label} onChange={e => { setHosts(hosts.map(h => h.id === host.id ? {...h, label: e.target.value} : h)); setSaved(false); }}/></label><p className="muted">{host.endpoints.map(url => new URL(url).host).join(" · ") || "Encrypted relay"}</p><small>Host ID: {host.id}</small></div><button type="button" aria-label={`Remove ${host.label}`} onClick={() => { setHosts(hosts.filter(h => h.id !== host.id)); setSaved(false); }}>Remove</button></li>)}</ul>
        </section>
