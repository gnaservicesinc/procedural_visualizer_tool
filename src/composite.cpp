        ...
        for (std::size_t index = 0U; index < project.layers.size(); ++index) {
            ...
            if (contributing) {
                ...
                if (erases_lower_layers) {
                    ...
                    // an exact no-op unless an effect synthesizes coverage; an
                    // ordered finishing stage can still clear it again.
                    // A later ordinary opaque layer may establish coverage again.
                    if (!detail::eraser_source_is_guaranteed_transparent(
                            render)) {
                ...
                if (!erases_lower_layers) {
                    enabled_stack_is_guaranteed_opaque =
                        enabled_stack_is_guaranteed_opaque
                        || (layer.opacity == 1.0
                            && !detail::render_data_can_create_transparency(
                                layer.render));
                }
