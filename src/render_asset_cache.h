...
// The editor also prunes individual caches when applying resource limits, so
// every pruning entry point must be visible in shared-library builds.
...
PVT_API void prune_obj_mesh_cache(const AssetPaths& objects);
PVT_API void prune_opengl_mesh_cache(const AssetPaths& heights);
